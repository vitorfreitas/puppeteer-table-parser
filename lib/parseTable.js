...
exports.parseTableFactory = void 0;
const helpers_1 = require("./helpers");
const parseTableFactory = (settings) => {
    const extraColsMapper = helpers_1.extraColsMapperFactory(settings.extraCols);
    ...
    return async (table, addHeader) => {
        ...
        const rows = (await table.$$('tbody:nth-of-type(2) tr')) || (await table.$$('tr'));
        ...
        const allowedIndexes = Object.fromEntries((await headerRow.$$eval('td,th', 
        // @ts-ignore
        (cells, newLine) => {
            return cells.map((cell) => cell.innerText.split(newLine));
        }, settings.newLine))
            .map((text, realIndex) => {
            const colName = settings.colFilter(text, realIndex);
        ...
        const colKeyToIndexWithExcluded = new Map();
        ...
        const finalRows = (await Promise.all(rows.map((row) => row.$$eval('td', 
        // @ts-ignore
        (cells, allowedIndexes) => {
            return cells
                .map((cell, realIndex) => [cell, realIndex])
                .filter((_, realIndex) => allowedIndexes[realIndex] !== undefined)
                .sort((a, b) => {
                const indexA = allowedIndexes[a[1]];
                const indexB = allowedIndexes[b[1]];
        ...
        if (addHeader) {
            ...
            finalRows.unshift(sortedHeader
                .filter((_, index) => !excludedKeyIndexes.includes(index))
                .join(settings.csvSeparator));
        }
        return finalRows;
...
exports.parseTableFactory = parseTableFactory;
